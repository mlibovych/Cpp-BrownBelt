...
public:
    ...
    ReadAccess At(const K& key) const {
        if (Has(key)) {
            ...
            return {lock_guard(part.m), part.data.at(key)};
        }
        throw std::out_of_range("Value is not present");
    }
...
private:
    ...
    size_t getSubmap(const K& key) const {
