...
ostream& operator<<(ostream& output, const HttpCode& h) {
    std::unordered_map<HttpCode, std::string> hash {
        {HttpCode::Ok, "OK"},
...
class HttpResponse {
    ...
    string m_content;
public:
    explicit HttpResponse(HttpCode code) : m_code(code) {
        //
    }
...
ostream& operator << (ostream& output, const HttpResponse& resp) {
    output << "HTTP/1.1 " << static_cast<int>(resp.m_code) << " " << resp.m_code << endl;
    for (const auto& header : resp.m_headers) {
        output << header << endl;
    }
    if (!resp.m_content.empty()) {
        output << HttpHeader{"Content-Length", to_string(resp.m_content.size())} << endl;
    }
    output << endl;
    if (!resp.m_content.empty()) {
    ...
    HttpResponse ServeRequest(const HttpRequest& req) {
        ...
        if (req.method == "POST") {
            if (req.path == "/add_user") {
                ...
                res.SetCode(HttpCode::Ok).SetContent(response);
            }
            ...
            if (req.path == "/user_comments") {
                ...
                res.SetCode(HttpCode::Ok).SetContent("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
            }
        }
        return res;
    }
...
//     // CommentServer cs;
//     // cs.ServeRequest({"POST", "/add_user"}, cout);
// }
